...
calculate_gap_columns_among_chrom_mark <- function(chrom_mark_df){
	chrom_mark_ordered_by_appearance <- unique(chrom_mark_df$chrom_mark) # list of different chromatin marks, ordered by how they appear in the heatmap from left to right
	count_df <- chrom_mark_df %>% count(chrom_mark)
	count_df <- count_df[match(chrom_mark_ordered_by_appearance, count_df$chrom_mark),] # order the rows such that the chrom_mark are ordered based on chrom_mark_ordered_by_appearance
...
pheatmap_emission_annot_state_group <- function(emission_df, save_fn, state_annot_fn){
	...
	states_to_select <- paste0('S', state_annot_df$state) # list of states that have been rearranged. This will be used as column names that we will select from states_to_select
	emission_dist <- plot_df[-1] # distance matrix where we get rid of the first column which is chrom mark names, so that the pheatmap function can recognize all values as numeric and hence can calculate the distance between states and distance between chrom marks
	emission_dist <- emission_dist %>% select(states_to_select) # rearrange the states
	...
	hm_no_grouping <- pheatmap(t(emission_dist), breaks = emission_value_ranges, fontsize = 5, annotation_col = chrom_mark_df, annotation_row = state_annot_df, annotation_colors = list(chrom_mark = CHROM_MARK_COLOR_CODE, GROUP = CELL_GROUP_COLOR_CODE, state_type = STATE_COLOR_DICT), gaps_row = gap_row_indices, cluster_rows = FALSE, cluster_cols = FALSE, show_colnames = FALSE, fontsize_col = 3, angle_col = 90 , cellheight = 5, filename = paste0(save_fn, ".png"))
	return (hm_no_grouping)
