...
###############################################################
# Submodular Selection of Assays (SSA-past)
###############################################################
...
def main(argv):
        ...
        parser = argparse.ArgumentParser(description="This script implements the Submodular Selection of Assays (SSA-past) method for selecting a panel of genomics assays for a cell type.  Takes as input a cell type, a list of assay names performed on the cell type and a matrix of assay-assay similarity values.  Outputs an ordered list of assay types, where the top K items in the list is the chosen panel of size K.")
        parser.add_argument("--cellType", required=True, help="Define the cell type, on which SSA-past is going to be performed. We support the following cell types: A549 Dnd41 GM12878 GM12891 GM12892 H1-hESC HMEC HSMM HSMMtube HUVEC HeLa-S3 HepG2 K562 MCF-7 NH-A NHDF-Ad NHEK NHLF Osteobl SK-N-SH.")
        ...
        ########################################
        print ("")
        dictCellTypes = {"A549", "Dnd41", "GM12878", "GM12891", "GM12892", "H1-hESC", "HMEC", "HSMM", "HSMMtube", "HUVEC", "HeLa-S3", "HepG2", "K562", "MCF-7", "NH-A", "NHDF-Ad", "NHEK", "NHLF", "Osteobl", "SK-N-SH"}
        if args.cellType not in dictCellTypes:
            raise Exception("ERROR: The input cell type {0} is not supported".format(args.cellType))
        ...
        try:
            f = open("data_matrices/"+args.cellType+"_assay_names.txt", 'r')
        except IOError:
            raise Exception('ERROR: Could not open the assay names file: {0}'.format("data_matrices/"+args.cellType+"_assay_names.txt"))
            sys.exit(1)
        ...
        ########################################
        try:
            f = open("data_matrices/"+args.cellType+"_similarity_matrix.txt", 'r')
        except IOError:
            raise Exception('ERROR: Could not open the file: {0}'.format("data_matrices/"+args.cellType+"_similarity_matrix.txt"))
            sys.exit(1)
